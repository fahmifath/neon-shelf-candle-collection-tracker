...
export function validate(input: CandleInput): ValidationErrors {
  ...
  if (!BURN_STATUSES.includes(input.burnStatus as BurnStatus)) {
    errors.burnStatus = "Invalid burn status.";
  }

  if (!VIEWS.includes(input.view as View)) {
...
export function createCandle(input: CandleInput, id: string, now: string): Candle {
  ...
  return {
    ...
    burnStatus: input.burnStatus as BurnStatus,
    rating,
...
export function normalize(raw: unknown): Candle | null {
  ...
  const input: CandleInput = {
    ...
    rating: r.rating !== undefined ? (r.rating as string | number | null) : null,
